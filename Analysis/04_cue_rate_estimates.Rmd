...
#
# Computes the variance of a weighted mean fottowing Cochran 1977 definition
...
sampledata11<-sampledata%>%
  ...
  summarise(ck=round(mean(clicks,na.rm=T),2),tc=unique(sumone),tt=unique(ll),rk=round((ck*tc)/tt,2),sd=round(sd(clicks,na.rm=T),2),CV=round(sd/ck,2),
              lower = round(ck-qt(0.975,df=n()-1)*sd/sqrt(n()),2),
              upper = round(ck+qt(0.975,df=n()-1)*sd/sqrt(n()),2),`95% CI`= paste0("[",lower, ",",upper,"]",sep=""))
# tt is the full length of the tag, sumone is the length of the time spent clicking


sampledata11[,c("Narwhal.ID", "tt", "tc", "ck", "rk", "CV", "95% CI")]



durdata<-sampledata11[,c("Narwhal.ID","tt")]
```
...
xbar <- mean(sampledata11$rk,na.rm=T) 
s <- sd(sampledata11$rk,na.rm=T)
...
```{r}
wxbar<-  weighted.mean(sampledata11$rk,sampledata11$tt,na.rm=T) 
ws <- sqrt(var.wtd.mean.cochran(sampledata11$rk,sampledata11$tt))
